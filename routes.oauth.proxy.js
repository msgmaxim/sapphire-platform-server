...
// scopes = pass through
const authmakecallback = function(type, app) {
...
module.exports.setupoauthroutes = function(app, db) {
  ...
  app.get('/oauth/redirect_uri', function(req, resp) {
    // check cookie first, if no session
    // then don't bother the backend server...
    const ses_id = req.cookies.get('altapi')
    // console.log('ses_id: '+ses_code);
    const ses = sessions[ses_id]
    if (!ses) {
