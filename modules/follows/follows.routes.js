...
module.exports = {
  mount: function(prefix, app) {
    ...
    function followHandler(req, resp) {
      ...
      const followsid = req.params.user_id
      dispatcher.getUserClientByToken(req.token, function(err, usertoken) {
        if (err) console.error('follows.routes.js::followHandler - getUserClientByToken err', err)
    ...
    app.delete(prefix + '/users/:user_id/follow', function(req, resp) {
      ...
      dispatcher.getUserClientByToken(req.token, function(err, usertoken) {
        if (err) console.error('follows.routes.js::deleteUSERSxFOLLOW - getUserClientByToken err', err)
        //console.log('usertoken', usertoken)
        if (usertoken == null) {
          // could be they didn't log in through a server restart
          const res = {
    ...
    app.get(prefix + '/users/:user_id/following', function(req, resp) {
      dispatcher.getUserClientByToken(req.token, function(err, usertoken) {
        if (err) console.error('follows.routes.js::getUSERSxFOLLOWING - getUserClientByToken err', err)
        if (usertoken === null) {
          const res = {
            meta: {
              code: 401,
    ...
    app.get(prefix + '/users/:user_id/followers', function(req, resp) {
      dispatcher.getUserClientByToken(req.token, function(err, usertoken) {
        if (err) console.error('follows.routes.js::getUSERSxFOLLOWERS - getUserClientByToken err', err)
        if (usertoken === null) {
          const res = {
            meta: {
              code: 401,
