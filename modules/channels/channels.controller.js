module.exports = {
  ...
  apiToChannel: function(api, meta, callback) {
    ...
    let raccess = 2 // 0=public, 1=loggedin, 2=selective
    let waccess = 2 // 1=loggedin, 2=selective
  ...
  channelToAPI: function(channel, params, tokenObj, callback, meta) {
    ...
    const api = {
      ...
      readers: {
        any_user: parseInt(channel.reader) === 1,
        immutable: !channel.readedit,
        public: parseInt(channel.reader) === 0,
        user_ids: channel.readers === null ? [] : channel.readers.split(/,/),
        you: false
      },
      editors: {
        any_user: false,
        immutable: !channel.editedit,
        public: false,
        user_ids: channel.editors === null ? [] : channel.editors.split(/,/),
        you: false
      },
      writers: {
        any_user: parseInt(channel.writer) === 1,
        immutable: !channel.writeedit,
        public: false,
    ...
    let idx = api.readers.user_ids.indexOf('')
    if (idx !== -1) {
    ...
    if (api.writers.user_ids) {
      const nList = []
      for (const i in api.writers.user_ids) {
        const writer = parseInt(api.writers.user_ids[i])
    ...
    this.cache.getChannelSubscriptionCount(channel.id, function(err, count) {
      if (err) console.error('channel.controller.js::channelToAPI(' + channel.id + ') - get sub count err', err)
    ...
    if (tokenObj.userid) {
      // process readers
      if (parseInt(channel.reader) === 0) {
        api.readers.you = true
      } else
      // logged in
      if (parseInt(channel.reader) === 1) {
        api.readers.you = true
      }

      if (parseInt(channel.writer) === 1) {
        api.writers.you = true
      }
      if (!api.writers.you) { // optimization
        const writes = channel.writers ? channel.writers.split(/,/) : []
    ...
    const mParams = { pageParams: { count: 1 }, generalParams: {}, tokenobj: tokenObj }
  ...
  deactiveChannel: function(channelid, params, token, callback) {
    ...
    if (isNaN(id)) {
      ...
      callback(new Error('channels.controller::deactiveChannel - isNaN'), false)
      return
    }
    ...
    this.getChannel(channelid, params, function(err, channel, meta) {
      ...
      ref.cache.getChannelSubscriptions([channelid], { }, function(err, subs, meta) {
        if (err) console.error('channel.controller.js::deactiveChannel - getChannelSubscriptions', err)
        for (const i in subs) {
          const userid = subs[i].userid
          ref.cache.setSubscription(channelid, userid, true, new Date(), function(err, subscription) {
            if (err) console.error('channel.controller.js::deactiveChannel - setSubscription', err)
  ...
  checkChannelAccess: function(channel, userid) {
    let allowed = false
    if (parseInt(channel.reader) === 0) allowed = true
    else if (userid) {
      if (channel.ownerid === parseInt(userid)) allowed = true
      else if (parseInt(channel.reader) === 1) allowed = true
      else if (parseInt(channel.reader) === 2) {
        const readList = channel.readers.split(/,/)
      ...
      if (!allowed && channel.writers) {
        const writeList = channel.writers.split(/,/)
  ...
  checkWriteChannelAccess: function(channel, userid) {
    ...
    if (userid) { // have to be loggedin to write
      if (channel.ownerid === parseInt(userid)) allowed = true
      else if (parseInt(channel.writer) === 1) allowed = true
      else if (parseInt(channel.writer) === 2) {
        const writeList = channel.writers.split(/,/)
        if (writeList.indexOf(userid + '') !== -1) {
          allowed = true
        }
  ...
  getUserChannels: function(params, tokenobj, callback) {
    ...
    if (!tokenobj.userid) {
      callback(new Error('not user token'))
      return
    }
    const ref = this
    ...
    this.cache.getUserChannels(tokenobj.userid, params, function(err, channels, meta) {
      ...
      for (const i in channels) {
        ...
        ref.channelToAPI(channel, params, params.tokenobj ? params.tokenobj : {}, function(cErr, api, meta2) {
  ...
  getChannel: function(ids, params, callback) {
    if (ids === undefined || ids === 'undefined') {
      ...
      callback(new Error('ids was undefined'))
      return
    }
    //ids = parseInt(ids)
    ...
    this.cache.getChannel(ids, params, function(err, channels, meta) {
      ...
      if (channels instanceof Array) {
        ...
        for (const i in channels) {
          ...
          */
          const allowed = ref.checkChannelAccess(channel, params.tokenobj ? params.tokenobj.userid : 0)
      ...
      const allowed = ref.checkChannelAccess(channels, params.tokenobj ? params.tokenobj.userid : 0)
      // block if not allowed
      if (!allowed) {
  ...
  channelSearch: function(criteria, params, tokenObj, callback) {
    ...
    this.cache.searchChannels(criteria, params, function(err, channels, meta) {
      ...
      for (const i in channels) {
        ...
        const allowed = ref.checkChannelAccess(channel, tokenObj.userid)
        // block if not allowed
        if (!allowed) {
          apis.push({ id: channel.id, readers: { user_ids: [] }, writers: { user_ids: [] }, editors: { user_ids: [] } })
          if (channels.length === apis.length) {
