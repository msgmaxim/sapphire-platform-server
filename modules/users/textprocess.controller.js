...
module.exports = {
  ...
  textProcess: function(text, entities, postcontext, callback) {
    ...
    const finishcleanup = function(html, text, callback) {
      if (!entities) {
        entities = {
          mentions: [],
          hashtags: [],
      ...
      if (!entities.hashtags || !entities.hashtags.length) {
        ...
        while ((match = hashtagRegex.exec(text))) {
          ...
          for (const i in entities.links) {
            const link = entities.links[i]
            const start = link.pos
            const end = start + link.len
      ...
      */
      ...
      // unicode chars
      // <>\&
      //console.log('textprocess.controller.js::textProcess - html unicoding', html)
      ...
      // remove line breaks
      //console.log('textprocess.controller.js::textProcess - html line breaking', html)
      html = html.replace(/\r/g, '&#10;')
      html = html.replace(/\n/g, '<br>')

      //console.log('textprocess.controller.js::textProcess - html wrapping', html)
      const res = {
        entities: entities,
    ...
    const mentionsSrch = text.match(mentionRegex)
    let launches = 0; let completed = 0
    if (mentionsSrch && mentionsSrch.length) {
      for (const i in mentionsSrch) {
        const mention = mentionsSrch[i] // with @
        //RegExp.$1 // without @
        //var username=RegExp.$1
        const username = mention.substr(1)
        //console.log("Replacing "+username)
        const pattern = new RegExp(' data-mention-name="' + username, 'gi')
        launches++
        ...
        if (userlookup[username]) {
          ...
          //console.log('textprocess.controller.js::textProcess - Searching for', username)
          ref.cache.getUserID(username, function(userErr, user, userMeta) {
            if (user) {
              //console.log('textprocess.controller.js::textProcess -got', user.id)
              // save in cache
