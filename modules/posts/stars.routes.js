  ...
  mount: function(prefix, app) {
    ...
    app.get(prefix + '/users/:user_id/interactions', function(req, resp) {
      ...
      dispatcher.getUserClientByToken(req.token, function(err, usertoken) {
        if (err) console.error('stars.routes.js::getUSERSxINTERACTIONS - getUserClientByToken err', err)
    ...
    function starHandler(req, resp) {
      ...
      dispatcher.getUserClientByToken(req.token, function(err, usertoken) {
        if (err) console.error('stars.routes.js::starHandler - getUserClientByToken err', err)
        if (usertoken == null) {
    ...
    app.delete(prefix + '/posts/:post_id/star', function(req, resp) {
      // req.apiParams.tokenobj isn't set because IO
      dispatcher.getUserClientByToken(req.token, function(err, usertoken) {
        if (err) console.error('stars.routes.js::deletePOSTSxSTAR - getUserClientByToken err', err)
        if (usertoken == null) {
    ...
    app.get(prefix + '/users/:user_id/stars', function(req, resp) {
      ...
      dispatcher.getUserClientByToken(req.token, function(err, usertoken) {
        if (err) console.error('stars.routes.js::getUSERSxSTARS - getUserClientByToken err', err)
