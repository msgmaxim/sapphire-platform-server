...
module.exports = {
  ...
  checkConnections: function() {
    var ref = this
    this.cache.getAllUserStreams(function(err, res) {
      if (err) console.error('stream.js::checkConnections - getAllUserStreams err', err)
      // send pings
      var wrap = {
        meta: {
  ...
  expireConnections: function() {
    var ref = this
    this.cache.getAllUserStreams(function(err, res) {
      if (err) console.error('stream.js::expireConnections - getAllUserStreams err', err)
      for (var i in res.userStreams) {
        var stream = res.userStreams[i]
  ...
  loadSubscriptions: function() {
    //console.log('streams::loadSubscriptions - start');
    ...
    var ref = this
    this.cache.getAllUserStreams(function(err, res) {
      if (err) console.error('stream.js::loadSubscriptions - getAllUserStreams err', err)
      //console.log('streams::loadSubscriptions - res', res);
      var tokenLookup = {}
      for (const i in res.tokens) {
        var tokenObj = res.tokens[i]
        tokenLookup[tokenObj.id] = tokenObj
      }
      var subLookup = {}
      for (const i in res.subs) {
        const sub = res.subs[i]
      ...
      console.log('streams::loadSubscriptions -', res.userStreams.length, 'streams')
      for (var i in res.userStreams) {
        var stream = res.userStreams[i]
        //console.log('streams::loadSubscriptions -', subLookup[stream.id].length, 'subcriptions for', stream.id, 'streamid');
        ...
        for (var j in subLookup[stream.id]) {
          const sub = subLookup[stream.id][j]
          ...
          switch (sub.stream) {
            case '/posts/stream':
              ...
              ref.cache.getFollowing(stream.userid, {}, function(err, followings) {
                if (err) console.error('stream.js::loadSubscriptions - getFollowing err', err)
                console.log('streams::loadSubscriptions -', stream.userid, 'is following', followings && followings.length, 'users', stream.connection_id)
            ...
            case '/channels':
              ...
              ref.cache.getUserSubscriptions(stream.userid, {}, function(err, subscriptions) {
                if (err) console.error('stream.js::loadSubscriptions - getUserSubscriptions err', err)
              ...
              break
            case '/users/me/posts': {
              const key = 'user.' + stream.userid + '.post'
              if (ref.dispatcher.pumps[key] === undefined) {
                ref.dispatcher.pumps[key] = []
              }
              ref.dispatcher.pumps[key].push(stream.connection_id)
            ...
            default: {
              if (sub.stream.match(/^\/channels\/\d+\/messages/)) {
                ...
                //console.log('streams::loadSubscriptions - \/channels\/\d+\/messages channelid', channelid);
                const key = 'channel.' + channelid + '.message'
                if (ref.dispatcher.pumps[key] === undefined) {
                  ref.dispatcher.pumps[key] = []
                }
                ref.dispatcher.pumps[key].push(stream.connection_id)
              } else {
                console.log('streams::loadSubscriptions - implement me', sub.stream)
              }
              break
            }
          }
      ...
      console.log('streams::loadSubscriptions - final keys', keys.length)
  ...
  checkConnection: function(stream, wrap) {
    ...
    setTimeout(function() {
      //console.log('checking pings for', stream.connection_id, ref.openPings[stream.connection_id].length);
      var cur = ref.openPings[stream.connection_id].indexOf(wrap.data.id)
      if (cur !== -1) {
        //console.log(wrap.data.id, 'ping is still outstanding for', stream.connection_id, 'state', stream.connection_closed_at);
        if (stream.connection_closed_at === null) {
          console.log('streams::checkConnections - marking offline', stream.connection_id)
          module.exports.cache.userStreamUpdate(stream.connection_id, {
            connection_closed_at: new Date()
          }, function(err, userStream) {

            //console.log('userStream updated')
          })
        }
      } else {
        // if has connection_closed_at, we need to clear it
        //console.log('streams::checkConnections - ', stream.connection_id, 'state', stream.connection_closed_at);
        if (stream.connection_closed_at !== null) {
          console.log('streams::checkConnections - marking online', stream.connection_id)
          module.exports.cache.userStreamUpdate(stream.connection_id, {
  ...
  checkExpire: function(stream) {
    ...
    if (!stream.auto_delete || stream.connection_closed_at === null) {
      //console.log('stream::checkExpire - skipping, AD:', stream.auto_delete, 'state', stream.connection_closed_at);
      return
    }
    var now = Date.now()
    ...
    var diff = now - then
    //console.log('streams::checkExpire - diff', diff, 'ms')
    if (diff < 10 * 60 * 1000) {
      return
    }
    console.log('streams::checkExpire - disconnected over 10mins ago', stream.connection_id, '@', stream.id)
    this.cache.deleteUserStream(stream.id)
  },
  handlePublish: function(connectionId, data) {
    //console.log('streams::handlePublish - start', connectionId, data);
    //try {
    this.redisClient.publish(connectionId, JSON.stringify(data))
    /*
    } catch(e) {
      console.log('cant publish', e)
    } */
  },
  pendingReloads: 0,
  handleEvent: function(channel, message) {
    // these could be HTTP requests
    //console.log('streams::handleEvent - chan', channel, 'msg', message);
    var ref = module.exports
    if (channel === 'AppDotNetAPI') {
      var pkt = JSON.parse(message)
      //console.log('streams::handleEvent - AppDotNetAPI - message', pkt)
      if (pkt.act === 'sub') {
        // uid,sid,sub {
        //   user_stream_id
        //   stream
        ...
        ref.pendingReloads++
        //console.log('streams::handleEvent - AppDotNetAPI - ask for reload')
        ...
        ref.reloadTimer = setTimeout(function() {
          console.log('streams::handleEvent - AppDotNetAPI - reloading, estimated new subs:', ref.pendingReloads)
          ref.loadSubscriptions()
    ...
    if (message.match(/^disconnect_/)) {
      ...
      //console.log(connectionId, 'disconnected, write to cache');
      ...
      if (!module.exports.openPings[connectionId]) {
        ...
        //console.log('streams::handleEvent - cant find any open pings for', connectionId);
        return
      }
      var idx = module.exports.openPings[connectionId].indexOf(timestamp)
      if (idx) {
        module.exports.openPings[connectionId].splice(idx, 1)
        //console.log('streams::handleEvent -', connectionId, 'is connected');
      } else {
        console.error('streams::handleEvent - no openping for', timestamp, 'for', connectionId, 'we have', module.exports.openPings[connectionId])
      }
    } else {
      console.error('streams::handleEvent - chan', channel, 'msg', message)
    }
  },
  ...
  handleSubscription: function(req, res) {
    ...
    //console.log('streams::handleSubscription - handling',req.path);
    this.redisClient.get('token_' + connId, function(err, reply) {
      if (err) console.error('streams::handleSubscription - redisClient token_ err', err)
      if (!reply) {
        sendErrResponse(500, 'no such connection')
        console.log('streams::handleSubscription - reply is empty', reply)
        return
      }
      connToken = reply.toString()
      //console.log('connToken', connToken);
      if (connToken !== accessToken) {
        console.log('streams::handleSubscription - connToken does not match accessToken', connToken, accessToken)
        sendErrResponse(401, 'token mismatch')
        return
      }
      ref.redisClient.get('autoDelete_' + connId, function(err, reply) {
        if (err) console.error('streams::handleSubscription - redisClient autoDelete_ err', err)
        var autoDel = reply.toString() // FIXME: look up in redis
        // convert string to token numeric id and get user id
        ref.cache.getAPIUserToken(accessToken, function(err, tokenObj) {
          //console.log('app::handleSubscription - tokenObj', tokenObj);
          if (err || !tokenObj) {
            sendErrResponse(401, 'invalid token')
            console.log('streams::handleSubscription - token not found', accessToken, err)
            return
          }
          ref.cache.findOrCreateUserStream(connId, tokenObj.id, tokenObj.userid, autoDel, function(err, userStream) {
            if (err || !userStream) {
              sendErrResponse(500, 'cant create userStream')
              console.log('streams::handleSubscription - userStream could not be created', connId, tokenObj.id, tokenObj.userid, autoDel, err)
              return
            }
            //console.log('app::handleSubscription - userStream', userStream);
            //console.log('app::handleSubscription - userStream.id', userStream.id);
            ...
            */
            ...
            if (!validEndpoint) {
              console.log('app::handleSubscription - endpoint', matchedPoint, 'is not subscribable/implemented yet')
              sendErrResponse(500, 'unsubscribable endpoint')
              return
            }
            //console.log('app::handleSubscription - checking for subscribable endpoints', req.path, userStream);
            ...
            // otherwise create it
            //console.log('app::handleSubscription - streamId', userStream.id);
            ref.cache.findOrCreateUserSubscription(userStream.id, req.path, JSON.stringify(req.apiParams), function(err, subscription) {
              if (err) console.error('streams::handleSubscription - err', err)
              if (err || !subscription) {
                console.log('streams::handleSubscription - subscription could not be created', userStream.id, req.path, err)
                sendErrResponse(500, 'cant create subscription')
                return
              }
              if (subscription.stream !== req.path) {
                console.log('streams::handleSubscription - subscription could not be created, path doesnt match', subscription.stream, '!=', req.path)
              }
              //console.log('streams::handleSubscription - connection', connId, 'endpoint', req.path, 'subscription', subscription)
              // poke the dispatch engine to reload state from db

              //ref.loadSubscriptions();
              //console.log('streams::handleSubscription - publish');
              var data = {
                act: 'sub',
                uid: tokenObj.userid,
                sid: userStream.id,
