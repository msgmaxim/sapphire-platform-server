const fs    = require('fs')
const path  = require('path')
const nconf = require('nconf')

const config_path = path.join(__dirname, '../config.json')
// Look for a config file
nconf.argv().env('__')
console.log('lib.config - config-file-path', nconf.get('config-file-path') || config_path)
if (nconf.get('config-file-path') && !fs.existsSync(nconf.get('config-file-path'))) {
  console.warn('lib.config - config-file-path is set and yet missing...')
  process.exit()
}
//nconf.argv().env('__').file('', nconf.get('config-file-path') || config_path)
nconf.file('', nconf.get('config-file-path') || config_path)

function moduleEnabled(name) {
  let val = nconf.get('modules:' + name)
  if (val === undefined) val = true
  return val
}

function isQuiet() {
  return nconf.get('logging:quiet') || false
}

function getLoggingPeridoticReports() {
  return !isQuiet() && (nconf.get('logging:peridotic_reports') || true)
}

module.exports = {
  moduleEnabled: moduleEnabled,
  nconf: nconf,
  getLoggingPeridoticReports: getLoggingPeridoticReports,
  isQuiet: isQuiet
}
